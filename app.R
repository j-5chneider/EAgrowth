...
server <- function(input, output, session) {
  ...
  for (i in seq_len(nrow(m_aw))) {
    m_aw[i,] = sprintf('<input type="radio" name="%s" value="%s"/>',
                       awareness[i],
                       m_aw[i,])
  }
  ...
  for (i in seq_len(nrow(m_in))) {
    m_in[i,] = sprintf('<input type="radio" name="%s" value="%s"/>',
                       inclination[i],
                       m_in[i,])
  }
  ...
  output$plot1 <- renderPlotly({
    ...
    plot_ly(
      ...
      layout(
        title = 'Your awareness and inclination scores over time',
  ...
  observe({
    ...
    updateSelectInput(session,
                      "showGroup",
                      "You see data of the group:",
                      selected = input$addToGroup)
  })
...
ui <-
  navbarPage(
    title = div(icon("lightbulb-o", lib = "font-awesome")),
        ...
        wellPanel(
          ...
          sliderInput(
            ...
            animate = animationOptions(
              interval = 1500,
              loop = T,
              playButton = "► automatisch abspielen"
            ),
            ticks = T,
            sep = ""
          )
